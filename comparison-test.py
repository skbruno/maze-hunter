...
def game(algorithm='bfs'):
    ...
    if algorithm == 'bfs':
        ...
        move_function = dijkstra_move
    elif algorithm == 'astar':
        move_function = lambda p, t, w: a_star_move(p, t, w, water)
    else:
    ...
    while running:
        ...
        pygame.display.flip()
        pygame.time.wait(150)  # Slow down the game a bit
        steps += 1
...
def run_comparisons(num_runs):
    ...
    for _ in range(num_runs):
        ...
        treasures, final_score, total_time, steps = game(algorithm='astar')
        results['astar'].append((treasures, final_score, total_time, steps))
...
def plot_results(results):
    ...
    for i, metric in enumerate(metrics):
        ...
        astart_data = [result[i] for result in astart_results]
        ...
        plt.plot(range(len(astart_data)), astart_data, label='A*', marker='^')
...
if __name__ == "__main__":
    loop_runs = 40
    results = run_comparisons(loop_runs)
